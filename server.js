...
async function pushNextPredictionNow() {
  try {
    // 防重复：如果没有新区块处理过，跳过推送（避免备份定时器重复推）
    ...
    if (shouldPush) {
      const nextHeight = Math.ceil((state.lastKnownBlockHeight || 0) / 20) * 20;
...
async function processNewBlocks() {
  ...
  try {
    ...
    storage.saveState();
	    ...
	    // 下一期预测由 handleBlock 延迟1秒推送，不再单独安排备份定时器
...
async function handleBlock(block) {
  ...
  // 清除尚未触发的定时推送（新区块到了，旧预测作废）
  if (advancePushTimer) {
    clearTimeout(advancePushTimer);
    advancePushTimer = null;
  }
  
  // 立即推送下一期预测（不等定时器）
  // 用 setTimeout 延迟1秒让状态先保存
  setTimeout(() => {
    pushNextPredictionNow();
  }, 1000);
